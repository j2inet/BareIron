...
async function extractItemsAndBlocks () {
  ...
  const blockSource = JSON.parse(await fs.readFile(`${__dirname}/ServerExtraction/generated/reports/blocks.json`, "utf8"));

  // Get registry data for extracting item IDs
  const registriesJSON = JSON.parse(await fs.readFile(`${__dirname}/ServerExtraction/generated/reports/registries.json`, "utf8"));
...
async function convert () {
  ...
  const inputPath = __dirname + "/ServerExtraction/generated/data/minecraft";
  const outputPath = __dirname + "/BareIronMinecraftServer/registries.cpp";
  const headerPath = __dirname + "/BareIronMinecraftServer/registries.hpp";
...
`;
  ...
  console.log("Done. Wrote to `registries.cpp` and `registries.hpp`");
